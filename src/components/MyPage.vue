  ...
  <div class="profile-container" v-if="member">
    ...
    <div class="profile-details">
      ...
      <div class="profile-item">
        ...
        <!-- 패스워드는 JSON 데이터에 포함되지 않으므로, 임의로 마스킹 처리 -->
      </div>
      <div class="profile-item">
...
export default {
  setup() {
    ...
    const fetchMemberData = async () => {
      try {
        const response = await axios.get('http://localhost:3002/members');
        member.value = response.data.find(member => member.member_id === 1);
        //테스트를 위해 id값이 1인 멤버값 출력하도록 설정 추후에 변경 예정
      } catch (error) {
